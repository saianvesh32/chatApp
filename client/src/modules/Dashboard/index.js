...
//     window.open("http://localhost:3000/users/sign_in", "_self");
// };
...
//     const res = await fetch(
//       `http://localhost:8000/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`,
//       {
//         method: "GET",
//         headers: {
//           "Content-Type": "application/json",
...
const Dashboard = () => {
  ...
  const logout = () => {
    localStorage.removeItem('user:token');
    localStorage.removeItem('user:detail');
    window.open("/users/sign_in", "_self");
};
  console.log(messages,'messages');
  useEffect(()=>{
    setSocket(io('http://localhost:8080'))
  ...
  useEffect(() => {
    ...
    const fetchConversations = async () => {
      const res = await fetch(
        `https://chatapp-5-e47g.onrender.com/api/conversations/${loggedInUser.id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const res = await fetch(`https://chatapp-5-e47g.onrender.com/api/users/${user?.id}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const fetchMessages = async (conversationId, receiver) => {
    const res = await fetch(
      `https://chatapp-5-e47g.onrender.com/api/message/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const sendMessage = async (e) => {
    ...
    const res = await fetch(`https://chatapp-5-e47g.onrender.com/api/message`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div>
      <div className="w-screen flex flex-wrap justify-between">
        ...
        <div className="w-[25%] border border-black h-screen bg-light px-8 py-16 overflow-scroll">
          <button className="btn bg-teal-400" onClick={logout}>Logout</button>
          <div className="text-primary text-lg">People</div>
          <div>
            {users.length > 0 ? (
              users.map(({ userID, user }) => {
